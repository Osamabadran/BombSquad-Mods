...
class ScreenLanguageEnglish(object):
    ...
    avgPtsLang = 'avgPTS'
    kdLang = 'K/D'
    infoLang = '* you need to play at least 10 games to be listed !'
...
class ScreenLanguageTurkish(object):
    ...
    avgPtsLang = 'ortPUAN'
    kdLang = 'K/D'
    infoLang = '* sıralamaya girmek için, en az 10 oyun oynamalısınız !'
...
class MythBServerInfo(bs.TeamGameActivity):
    ...
    def onBegin(self):
        ...
        try:
            ...
            ftp = FTP_TLS('mythb.me')
            ftp.login('bombsquad','bombsquadftp00')
            ftp.prot_p() 
            ftp.cwd('/files/')
            dir = ftp.nlst()
            ...
            for i in data:  #check if player never were killed # check if player played more then 10 games  else make all scores zero / names -- check id none
             avgScore = data[i]['name_full'] if data[i]['played'] > 9 else \
             '--', float(data[i]['scores']) / (data[i]['played']) if data[i]['played'] > 9 else \
             0, i if data[i]['played'] > 9 else 'null'
             kdRatio = data[i]['name_full'] if data[i]['played'] > 9 else \
             '--', float(data[i]['kills']) / ( 1 if data[i]['killed'] == 0 else data[i]['killed']) if data[i]['played'] > 9 else \
             0, i if data[i]['played'] > 9 else 'null'
